...
const TreeBrowser = (props) => {
    ...
    const isActive = (item) => {
        ...
        const parentLocationInPathRegExp = new RegExp(`(/|^)${locationData.parentLocationId}(/)?$`);
        console.log(locationData.parentLocationId, item);
        // return item.id === locationData.location?.id ?? null;
        return parentLocationInPathRegExp.test(item.path);
    ...
    const onItemClick = (item) => {
        ...
        dispatchLoadedLocationsAction({
            type: 'SET_LOCATIONS',
            data: [{ parentLocationId: locationId, treeBrowserItemName: name, subitems: [] }],
        });
    };
